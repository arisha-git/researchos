...
export class ContextAssembler {
   ...
   */
  public static assemble(chunks: RetrievedChunk[]): string {
    if (chunks.length === 0) return "No relevant research context found.";

    return chunks
      .map((chunk, index) => {
        return `--- Evidence Node ${index + 1} ---
Source: ${chunk.chunk.document_name} (Page ${chunk.chunk.page_number})
Section: ${chunk.chunk.section_header}
Content: ${chunk.chunk.chunk_text}
`;
      })
      .join("\n");
  }
